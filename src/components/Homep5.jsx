  ...
  return (
    <div>
      <div className='w-full h-auto bg-[#131926] md:px-16 md:py-16 p-4'>
        <span className='text-xs uppercase text-indigo-600'>Selectable skills</span>
        <h1 class="text-4xl max-w-2xl md:text-5xl text-white font-bold tracking-tight mt-4 mb-4">
      Switch on what the model can do
    </h1>
    <p className='text-white/60 text-lg max-w-2xl mb-10'>
        Skills that build toward real mastery — connected to your workspace, on your terms.
    </p>
    ...
    <div key={index} className='border transition-all duration-500 ease-in-out hover:-translate-y-1 hover:scale-102 hover:shadow-2xl bg-[#0c101a] border-white/20 rounded-2xl p-4 flex flex-col gap-4 '> 
      ...
      <div className='mt-6'>
      <Link className='text-indigo-600 font-semibold hover:text-indigo-700 flex gap-2 items-center' to='/skills'>See all skills <ArrowRight className=' transition-all duration-300 hover:translate-x-1 ' size={15} /></Link>
      </div>
    </div>
    </div>
  )
